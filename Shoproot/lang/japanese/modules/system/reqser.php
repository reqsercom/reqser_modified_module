...
define('MODULE_SYSTEM_REQSER_REQSER_API_KEY_TITLE', 'APIキー');
define('MODULE_SYSTEM_REQSER_REQSER_API_KEY_DESC', '<i>Reqser.comの</i>APIキーをここに入力できます。ReqserでModifiedに接続する際に表示されます。<a href="https://www.reqser.com/translation/new-website/Modified" target="_blank" title="Direkt Link">直接リンクはこちら</a> ');
...
define('MODULE_SYSTEM_REQSER_ALLOW_ALL_ROW_ACCESS_TITLE', '<hr /><h2>言語テーブル</h2>Reqser.comに共有テーブルの全カラムへのアクセス権を与えますか？');
define('MODULE_SYSTEM_REQSER_ALLOW_ALL_ROW_ACCESS_DESC', '「はい」を選択すると、Reqserはリリースされたテーブルのすべての列エントリにアクセスできます。「いいえ」を選択すると、古いショップのバージョンや非標準のテンプレートを使用した場合にエラーが発生する可能性があります。');
...
define('MODULE_SYSTEM_REQSER_ADD_LANGUAGE_ALLOWED_DESC', 'この設定を「はい」にすると、新しい言語を<i><a href="https://reqser.com/" target="_blank">reqser.com</i></a> に追加できます。<br />いいえ "の場合、その言語はすでにショップシステムに存在し、このモジュールで有効化されている必要があります。<br />ショップに言語を作成する：<a href="' . DIR_WS_ADMIN . FILENAME_LANGUAGES . '">ショップに言語を作成しますか？</a> &nbsp;&nbsp;<span class="alrt">!!新しい言語は24-48時間後にショップで利用可能になります。!!</span><br />翻訳する言語としてここをマークしてください。');
...
define('MODULE_SYSTEM_REQSER_API_KEY_WRONG_ERR', 'このモジュールに保存されている "Reqser APIキー "が、<i>reqser.comに</i>保存されているものと一致しません。<br /><i>reqser</i>.comにログインしてAPIキーを確認するか、必要であれば再度取得してここに保存してください。');
